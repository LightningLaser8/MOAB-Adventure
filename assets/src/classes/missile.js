class Missile extends Bullet{
  ...
  step(dt){
    super.step(dt)
    if(this.target && !this.target.dead && !this.target.remove){ //If target still there
      this.rotateTowards(this.target.x, this.target.y, this.turnSpeed)
    }
    let selected = null
    if(this.targetType === "nearest"){ //Closest to bullet
      if(this.world){ //If the bullet exists
        let minDist = Infinity
        for(let entity of this.world.entities){
          if(entity.team !== this.entity.team && !entity.dead){//Only select living entities
            let dist = Math.sqrt((this.x - entity.x)**2 + (this.y - entity.y)**2) //Pythagorean Theorem to find distance
            if(dist < minDist){ //If closer
              selected = entity;
              minDist = dist;
            }
          }
    ...
    else if(this.targetType === "hovered"){ //Closest to mouse pointer
      if(this.world){ //If the bullet exists
        let minDist = Infinity
        for(let entity of this.world.entities){
          if(entity.team !== this.entity.team && !entity.dead){//Only select living entities
            let dist = Math.sqrt((ui.mouse.x - entity.x)**2 + (ui.mouse.y - entity.y)**2) //Pythagorean Theorem to find distance
            if(dist < minDist){ //If closer
              selected = entity;
              minDist = dist;
            }
    ...
    else if(this.targetType === "mouse"){ //Closest to mouse pointer
      selected = ui.mouse
    }
